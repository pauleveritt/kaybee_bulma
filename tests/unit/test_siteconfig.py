from kaybee_bulma.siteconfig import SiteConfig, Logo
...
class TestSiteConfig:
    ...
    def test_no_favicon(self):
        config_data = dict()
        config = SiteConfig(**config_data)
        assert None is config.favicon

    def test_favicon(self):
        config_data = dict(favicon='xyz.png')
        config = SiteConfig(**config_data)
        assert config_data['favicon'] is config.favicon
